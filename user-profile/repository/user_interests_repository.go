...
const (
	...
	UnFollowInterest     = "delete from user_interests where user_id = $1 and interest_id = $2"
)
...
type UserInterestsRepository interface {
	...
	UnFollowInterest(ctx *gin.Context, interestID, userID uuid.UUID) error
}
...
func (repository userInterestsRepository) UnFollowInterest(ctx *gin.Context, interestID, userID uuid.UUID) error {
	log := logging.GetLogger(ctx).WithField("class", "UserInterestsRepository").WithField("method", "UnFollowInterest")

	_, err := repository.db.ExecContext(ctx, UnFollowInterest, userID, interestID)
	if err != nil {
