...
deploy_dev: docker_login
	docker run --rm --name deploy $(ARTIFACTORY_USER)/gola-ansible:latest project=$(PROJECT) action=$(ACTION) inventory=$(INVENTORY) ssh_public_key=$(SSH_PUBLIC_KEY) ssh_private_key=$(SSH_PRIVATE_KEY)