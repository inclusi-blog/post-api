...
const (
	...
	FetchDraft       = "MATCH (draft:Draft)-[audit:CREATED_BY]->(author:Person) OPTIONAL MATCH (tags:Interest)<-[:FALLS_UNDER]-(draft) where draft.draftId = $draftId and author.userId = $userId return draft.tagline as tagline, draft.previewImage as previewImage, collect(tags.name) as interests, draft.data as postData, CASE WHEN exists(draft.isPublished) THEN draft.isPublished ELSE false END AS isPublished, audit.createdAt as createdAt"
